...
import argparse, sys, ujson
from collections import defaultdict

class traIXroute_parser():

    def __init__(self, exact_time):
        #self.inputIP: The destination IP or FQDN to send the probe.
        self.inputIP = ''
        # self.outputfile: The output file name in which all the results will be redirected to.
        self.outputfile = 'output'
        # self.inputfile: The file name with the list of the destination IP addresses/FQDNs to send the probes.
        self.inputfile = ''
        # self.arguments: traIXroute arguments.
        self.arguments = ''
        # self.flags: A dictionary containing the user input flags.
        self.flags = defaultdict(bool)
        # The timestamp when the traIXroute started to run for the first time.
    ...
    def parse_input(self):
        ...
        parser.add_argument('-db','--store-database', action='store_true',help='Enables printing the database information.')
        ...
        group_1.add_argument('-t','--traceroute', nargs='*', action='store', type=str,help='Calls traIXroute with traceroute and (optional) traceroute arguments.')
        group_1.add_argument('-s','--scamper', nargs='*', action='store', type=str,help='Calls traIXroute with scamper and (optional) scamper arguments.')

        group_2 = parser_ripe.add_mutually_exclusive_group(required=True)
        group_2.add_argument('-r','--request', nargs=1, action='store', type=ujson.loads,help='Fetches the traceroute results of a RIPE Atlas measurement to detect IXP crossing links.')
        group_2.add_argument('-c','--create', nargs=2, action='store', type=ujson.loads,help='Creates a new measurement in the RIPE Atlas platform with the desired options.')
        ...
        if options.output:
            ...
            self.outputfile=self.outputfile+'_'+self.exact_time+'.txt'
