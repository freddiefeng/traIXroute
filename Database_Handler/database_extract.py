...
class database():
    ...
    def __init__(self, traIXroute_parser, mypath, config, outcome):
        '''
        The initialization of the tool.
        Inputs: 
            a) traIXroute_parser: Dictionary that contains the user input flags.
            b) mypath: The directory path of the traIXroute folder.
        ...
        # self.final_sub2name: A dictionary with {Subnet}=[IXP long name, IXP short name].
        self.final_sub2name = None
        # self.reserved_sub_tree: A Subnet Tree with {reserved subnet}=reserved subnet.
        self.reserved_sub_tree = None
        # self.asn_memb: A dictionary with {ASN}=[list of IXP names].
        self.asnmemb = None
        # self.asn_routeviews: A Subnet Tree with {Subnet}=[ASN1, ASN2,...] from routeviews.
        self.asn_routeviews = None
        # self.subTree: A Subnet Tree with {Subnet}=[IXP long name, IXP short name].
        self.subTree = None
    ...
    def dbextract(self):
        ...
        if (lst_modified and not chk_update and not self.merge_flag) or not self.outcome:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.config["num_of_cores"]) as executor:
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'IXPIP2ASN.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'trIX_subnet2name.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'asn_memb.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'sub2country.json'))
                results.append(executor.submit(json_handle.import_IXP_dict, self.mypath + 'routeviews.json'))
            ...
            if not flag:
                ...
                if self.print_db:
                    output.print_pr_db_stats(self.mypath+'db.txt')
        ...
        if (not lst_modified or flag or chk_update or self.merge_flag ) and self.outcome:
            if chk_update or flag:
                ...
                if exists(self.mypath+"check_update.txt"):remove(self.mypath+"check_update.txt")
            ...
            results=[]
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.config["num_of_cores"]) as executor:
                results.append(executor.submit(asn_hand.routeviews_extract,self.route_filename,self.mypath,self.reserved_sub_tree,self.config,db_path))
                results.append(executor.submit(peeringdb.peering_handle,self.filename_peer_name,self.filename_peer_ip,self.filename_peer_pfx,self.filename_peer_ixlan,additional_info_tree,self.mypath,self.reserved_sub_tree,self.config,country2cc,db_path))
                results.append(executor.submit(pch.pch_handle_main,self.filename_ixp_membership,self.filename_subset,self.filename_excha,self.mypath,self.reserved_sub_tree,additional_info_tree,country2cc,db_path))
            ...
            [self.subTree,self.final_sub2name,final_subnet2country]=dict_merge.include_additional(self.final_sub2name,self.subTree,additional_subnet2name,final_subnet2country,additional_pfx2cc,help_tree)

            [merged_ixp2asn,dirty_count] = dict_merge.merge_ixp2asns(pch_ixp2asn,peering_ip2asn,True,self.subTree)

            self.final_ixp2asn = dict_merge.merge_ixp2asns(additional_ip2asn,merged_ixp2asn,False,self.subTree,replace = True)
            self.asnmemb = asn_hand_info.asn_memb(self.final_ixp2asn,self.subTree)
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.config["num_of_cores"]) as executor:
                executor.submit(json_handle.export_IXP_dict,self.asnmemb,self.mypath+'asn_memb.json')
                executor.submit(json_handle.export_IXP_dict,routeviews_dict,self.mypath+'routeviews.json')
                executor.submit(json_handle.export_IXP_dict,self.final_ixp2asn,self.mypath+'IXPIP2ASN.json')
                executor.submit(json_handle.export_IXP_dict,self.final_sub2name,self.mypath+'trIX_subnet2name.json') 
                executor.submit(json_handle.export_IXP_dict,final_subnet2country,self.mypath+'sub2country.json')  

            output.print_db_stats(len_peering_ip2asn,len_peering_subnet2name,len_pch_ixp2asn,len_pch_subnet2names,self.final_ixp2asn,self.final_sub2name,dirty_count,additional_ip2asn,additional_subnet2name,lenreserved,self.print_db,self.mypath)  
        ...
        if self.merge_flag:
            ...
            self.ips_to_file(additional_ip2asn,additional_info_tree,merged_ixp2asn,'ixp_membership.txt')
    ...
    def subs_to_file (self,d1,d2,filename):
        ...
        filename=open(self.traixpath+filename,'w')
        output=''
        number=0
        for key in d1.keys():
            ...
            if key in self.subTree:
                ...
                if key in self.cc_tree:
                    output=output+', '+self.cc_tree[key][1]+', '+self.cc_tree[key][0]
                output=output+'\n'
                number=number+1
        for key in d2.keys():
            ...
            if key not in d1.keys():
                if key in self.subTree:
                    ...
                    for IXP in self.subTree[key]:
                        output=output+', '+IXP[1]+', '+IXP[0]
                    if key in self.cc_tree:
                        output=output+', '+self.cc_tree[key][0]+', '+self.cc_tree[key][1]
    ...
    def ips_to_file(self,additional_ip2asn,additional_ip2name,merged_ixp2asn, filename):
        '''
        ...
        Input:
            ...
            d) filename: The output file name.
        '''
